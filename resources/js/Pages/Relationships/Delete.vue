<template>
    <form @submit.prevent="submit">
        <header class="px-6 mt-6">
            <h2 class="text-gray-700 text-xs uppercase tracking-wide font-semibold">Delete Relationship</h2>
        </header>
...
<script>
    ...
    export default {
        ...
        methods: {
            async submit() {
                try {
                    await this.form.delete(`/relationships/${this.relationship.id}`);

                    this.$emit("close");

                    EventBus.$emit('relationshipWasDeleted', true);
