<template>
	<form @submit.prevent="submit">
        <header class="px-6 mt-6">
            <h2 class="text-gray-700 text-xs uppercase tracking-wide font-semibold">Create Relationship Type</h2>
        </header>
...
<script>
	...
	export default {
		...
		methods: {
            async submit() {
				try {
                    await this.form.post(`/relationships`);

                    this.$emit("close");

                    EventBus.$emit('relationshipWasCreated', true);
