	...
	<layout title="The Expert - Users">
		<div class="w-2/3 mx-auto mb-12">
			<header class="flex justify-between my-8">
				<h1 class="text-lg font-semibold">Allegation Types</h1>
			...
			<div class="bg-white mt-6 shadow rounded overflow-hidden">
				<table class="text-left w-full">
					<thead>
						<tr>
							<th
								class="px-4 py-4 bg-gray-100 text-xs text-gray-600 uppercase font-semibold tracking-wide"
							>Name</th>
							<th
								class="px-4 py-4 bg-gray-100 text-xs text-gray-600 uppercase font-semibold tracking-wide"
							>Created at</th>
					...
					<tbody>
						<tr v-for="type in types" :key="type.id" class="border-t">
							<td class="px-4 py-4">{{ type.name }}</td>
							<td class="px-4 py-4">{{ type.created_at }}</td>
							<td class="px-4 py-4">{{ type.updated_at }}</td>
							<td class="px-4 py-4">
								<div class="flex items-center">
									<a href="#" class="text-xs uppercase tracking-wider text-gray-700" @click.prevent="handleEditing(type)">Edit</a>
									<span class="text-xs px-2 text-gray-400">|</span>
									<a href="#" class="text-xs uppercase tracking-wider text-gray-700" @click.prevent="handleDeleting(type)">Delete</a>
								</div>
							</td>
						</tr>
					</tbody>
...
<script>
	...
	export default {
        ...
        mounted() {
            EventBus.$on('allegationTypeWasCreated', (value) => {
                this.wasStored = value;
            });

            EventBus.$on('allegationTypeWasUpdated', (value) => {
                this.wasUpdated = value;
            });

            EventBus.$on('allegationTypeWasDeleted', (value) => {
		...
		methods: {
			handleCreating() {
				this.$modal.show(TypeCreate, {}, { height: "auto" });
			},
            handleEditing(allegationType) {
                this.$modal.show(TypeEdit, { allegationType }, { height: "auto" });
            },
            handleDeleting(allegationType) {
                this.$modal.show(TypeDelete, { allegationType }, { height: "auto" });
            }
		}
	};
</script>
